...
# ----------------------------------------------------------------------------------------
# FASE 4b: JOHANSEN COMPETITIVO A 3 VARIABILI SUL NUCLEO STABILE (Setaccio Strutturale)
# ----------------------------------------------------------------------------------------
print("\n[--- Sotto-Fase 4b: Johansen Competitivo (S1x, S2, S3) sull'Epoca d'Oro ---]")

from statsmodels.tsa.vector_ar.var_model import VAR
from statsmodels.tsa.vector_ar.vecm import coint_johansen

def confronto_johansen_3v(data, candidata_nome):
    # 0. Pulizia del dataset per la terna specifica
    data_clean = data.dropna()
    
    # 1. Ricerca del Lag Ottimale (AIC) sul nucleo stabile
    modello_var_temp = VAR(data_clean)
    lag_res = modello_var_temp.select_order(maxlags=5)
    p_ottimale = lag_res.aic
    
    # In Johansen, k_ar_diff corrisponde a p - 1
    k_ar_diff_joh = max(1, p_ottimale - 1)
    
    # 2. Esecuzione di Johansen (det_order=1 come validato in precedenza)
    joh_res = coint_johansen(data_clean, det_order=1, k_ar_diff=k_ar_diff_joh)
    
    trace_stat = joh_res.lr1
    crit_vals_95 = joh_res.cvt[:, 1]
    
    margine_primario = trace_stat[0] - crit_vals_95[0]
    
    print(f"\n  [ Sistema Termodinamico: {candidata_nome} - S2 - S3 ]")
    print(f"  - Lag ottimale (AIC) utilizzato: {p_ottimale}")
    print(f"  - Statistica Traccia (r=0): {trace_stat[0]:.4f}")
    print(f"  - Soglia Crit. (95%): {crit_vals_95[0]:.4f}")
    
    if margine_primario > 0:
        print(f"  -> Cointegrazione TROVATA. Margine di forza: +{margine_primario:.4f}")
    else:
        print(f"  -> Cointegrazione ASSENTE. Distanza dalla soglia: {margine_primario:.4f}")
        
    # 3. Test Sequenziale del Rango (con freno di sicurezza)
    r = 0
    for i in range(len(trace_stat)):
        if trace_stat[i] > crit_vals_95[i]:
            r = i + 1
        else:
            break # [!] Freno di blocco matematico per anomalie di rango [!]
            
    print(f"  -> Rango di cointegrazione stimato: r = {r}")
    
    return margine_primario

# Esecuzione autonoma sulle due terne prelevando i dati in automatico da df_stabile
margine_A = confronto_johansen_3v(df_stabile[['S1A', 'S2', 'S3']], "S1A")
margine_B = confronto_johansen_3v(df_stabile[['S1B', 'S2', 'S3']], "S1B")

# ----------------------------------------------------------------------------------------
# FASE 4c: ELEZIONE DELLA VARIABILE UFFICIALE E PROMOZIONE AL VECM
# ----------------------------------------------------------------------------------------
print("\n[ ESITO DEL CONFRONTO TERMODINAMICO DEFINITIVO ]")

var_vincente_definitiva = None

if margine_A > margine_B:
    print("» Il sistema stocastico più forte e coeso è guidato da S1A.")
    var_vincente_definitiva = 'S1A'
elif margine_B > margine_A:
    print("» Il sistema stocastico più forte e coeso è guidato da S1B.")
    var_vincente_definitiva = 'S1B'
else:
    print("» Pareggio. Si adotta S1B per la priorità strutturale dell'estrazione.")
    var_vincente_definitiva = 'S1B'

# Promozione globale nel dataframe originale e nel nucleo stabile
df['S1'] = df[var_vincente_definitiva]

# Utilizziamo loc per evitare il SettingWithCopyWarning su df_stabile
df_stabile = df_stabile.copy()
df_stabile.loc[:, 'S1'] = df_stabile[var_vincente_definitiva]

print(f"\n>>> L'elezione è conclusa. La variabile promossa al calcolo di k e al VECM è {var_vincente_definitiva}.")
print("-" *85)
...
restr_S1_causa_S2 = ", ".join([f"L{i}_S1 = 0" for i in range(1, p_ty + 1)])
restr_S3_causa_S1 = ", ".join([f"L{i}_S3 = 0" for i in range(1, p_ty + 1)])
...
# ==============================================================================
# FASE 10: TERMODINAMICA INTEGRALE (AREE DI LEGAME E DEFICIT STRUTTURALE)
# ==============================================================================
print("\n" + "="*40)
print("FASE 10: CALCOLO DELLE AREE INTEGRALI (TERMODINAMICA SOCIALE)")
print("="*40)
